...
class SemanticEncoder(nn.Module):
    ...
    def forward(self, texts: List[str]) -> torch.Tensor:
        ...
        for i in range(0, len(texts), self.batch_size):
            ...
            # 获取编码
            # 关键修复：即使freeze_encoder=True，也不应该用no_grad()
            # 冻结参数意味着参数不更新，但输出仍然可以有梯度，这样投影层才能训练
            # 只有当encoder参数requires_grad=False时，梯度不会回传到encoder参数，但会回传到投影层
...
class CrossAttentionFusion(nn.Module):
    ...
    def __init__(
        ...
        # 语义适配层：将语义表示适配到结构维度附近，保留更多语义信息
        # 如果语义维度和结构维度差异大，先适配到结构维度，再投影到output_dim
    ...
    def forward(
        ...
        # 3. 重塑为多头格式：[num_nodes, num_heads, head_dim]
